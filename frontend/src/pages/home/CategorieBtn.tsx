...
function CategorieBtn({type, selected, setSelected}:{type:RecipeType | "All", selected:boolean, setSelected:any}) {
    ...
    return ( 
        <button onClick={()=>setSelected(type)} className={`${selected&&selectedStyle} uppercase text-gray-500 font-bold rounded-full px-4 py-1 text-xs sm:text-base`}>{type}</button>
     );
}
